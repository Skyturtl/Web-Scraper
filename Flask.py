...
def extract_phrases_and_terms(query):
    ...
    # Extract quoted phrases (may contain wildcards *)
    quoted_phrases = re.findall(r'"(.*?)"', query)
    remaining_query = re.sub(r'"(.*?)"', '', query)
    
    # Split remaining query into tokens (terms and wildcards)
    tokens = re.findall(r'\*{1,3}|\b\w+\b', remaining_query.strip())
    
    # Separate phrases, wildcard sequences, and terms
    ...
    if current_sequence:
        wildcard_sequences = [current_sequence]
    
    terms = []
    for seq in wildcard_sequences:
        for item in seq:
            if item['type'] == 'terms':
                terms.extend(item['value'])
    
    return phrases, wildcard_sequences, terms, negative_terms
...
def calculate_tfidf(query,title_boost_factor=3):
    ...
    if wildcard_doc_sets and any(wildcard_doc_sets):
        ...
        has_special_criteria = bool(phrases)
